...
from mmdetection.mmdet.apis.inference import inference_single_attack_single_box as fcos_attack_box
...
def toTensor(img, device='cuda:0'):
    ...
    return img.float().to(device).unsqueeze(0)
                ...
                for class_name, same_class_box in same_class.items():
                    ...
                    for i in range(divide_number):    
                        for j in range(divide_number):
                            ...
                            boxes_mask , labels_mask = fcos_attack_box(img_path, fcos_model, img_mask, img_mask)
...
def parse_args():
    ...
    parser.add_argument('--save_iter_images', action='store_true',
                        help='Save intermediate adversarial image for each iteration')
    parser.add_argument('--no_save_final_images', action='store_true',
                        help='Do not save final adversarial images')
...
def attack_imgs(root_path, imgs):    
    ...
    for ind in range(len(imgs)):
        ...
        attack_map = None 
        img = None
        img_cv2 = toTensor(cv2.imread(img_path), args.device)
        ...
        with torch.no_grad():
            ...
            attack_map,attack_map_mean=make_init_mask_img(boxes_init,pred_init,labels_init,mask_layer,img_cv2_800_800, img_cv2,img_path, fcos_model)           
            #############################
        ...
        for attack_iter in pbar:
            ...
            fcos_noise, fcos_boxes,labels_pred,class_loss,iou_loss = fcos_attack_mt(img_path, fcos_model, img_cv2_800_800, img_cv2,init_det,iou_thre) 
            noise_img = np.sign(fcos_noise)
            if np.sum(np.isnan(noise_img))==original_img.size:
                break
            attack_rate =  attack_map[attack_map==1].size / attack_map.size
            output_str = imgs[ind] + '当前{}/{}'.format(ind, len(imgs)) + '次数{}'.format(attack_iter)+'最初：{}'.format(ori_bbox_num)+'当前fcos:{}'.format(len(fcos_boxes))+"当前攻击比例:{}".format(attack_rate)
            pbar.set_description(output_str)
            
            img_last = img_cv2.cpu().detach().clone().squeeze(0).numpy().transpose(1, 2, 0)
            del img_cv2
            a = noise_img.astype(np.float32) * attack_map
            a = a[...,::-1].copy()   
