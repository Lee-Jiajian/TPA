...
def attack_imgs(root_path, imgs):    
    ...
    for ind in range(len(imgs)):
        ...
        for attack_iter in pbar:
            ...
            output_str = imgs[ind] + '当前{}/{}'.format(imgs.index(imgs[ind]), len(imgs)) + '次数{}'.format(attack_iter)+'最初：{}'.format(ori_bbox_num)+'当前fcos:{}'.format(len(fcos_boxes))+"当前攻击比例:{}".format(attack_rate)
