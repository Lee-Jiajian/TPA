...
from mmdetection.mmdet.apis.inference import LoadImage, LoadImage_fcos
...
def make_cam_patch_mask(boxes_init, img_tensor_np, patch_type='rotated', max_area_ratio=0.5, attack_gradient=None):
    """
    基于梯度（Pseudo-CAM）生成补丁掩码
    Args:
        ...
        attack_gradient: 对抗梯度（FCOS模型反向传播得到，shape=(H,W,3)）
    Returns:
    ...
    # 生成显著性地图（梯度绝对值最大为显著，无梯度则用全1）
    if attack_gradient is not None:
        saliency_map = np.max(np.abs(attack_gradient), axis=2)  # 3通道转单通道
    else:
...
def attack_imgs(root_path, args, imgs):
    ...
    for ind, img_name in enumerate(imgs):
        if ind<=0:
            ...
            # ===================== 【核心修正1：锁死干净原图的初始检测】 =====================
            # 攻击前完成干净检测，结果全程独立保存，不被攻击过程修改
            fcos_boxes_resize, labels, init_det = fcos_attack_init(img_path, fcos_model, img_tensor, img_tensor)
            print(f"\n{'='*60}")
            print(f"图片索引: {ind}")
            print(f"图片名称: {img_name}")
            print(f"图片路径: {img_path}")
            print(f"初始检测框总数: {len(fcos_boxes_resize)}")
            print(f"置信度分布:")
            if fcos_boxes_resize.size == 0:
                print(f"警告：{img_name} 初始检测无舰船框，跳过")
                continue
            ...
            if fcos_model.cfg['model']['type']=='FCOS':
            ...
            print("fcos model", fcos_model)
            # out = fcos_model(img_tensor,img_metas=img_mm)
            out = fcos_model.forward_test([img_tensor],img_metas=img_mm)

            print("out", type(out), len(out), len(out[0]), len(out[1]), len(out[2]), len(out[3]) )

            print("out[0]", out[0][0][0])
            print("out[1]", out[1].shape)
            print("out[2]", out[2].shape)
            print("out[3]", out[3].shape)
            # 在第310行（out打印之后）添加：
            print("out", type(out), len(out), len(out[0]), len(out[1]), len(out[2]), len(out[3]))
            ...
            print(f"out[3] 数据类型: {out[3].dtype}")
            print(f"out[3] 数值范围: min={out[3].min().item():.3f}, max={out[3].max().item():.3f}")

            # 打印前5个点的值
            print("\nout[3] 前5个点的值:")
            for i in range(min(5, len(out[3]))):
                val1, val2 = out[3][i]
                print(f"  点{i}: [{val1:.3f}, {val2:.3f}]")

            # 检查第1列和第2列的关系
            col1 = out[3][:, 0]  # 第一列
            col2 = out[3][:, 1]  # 第二列

            print(f"\n第1列: min={col1.min().item():.3f}, max={col1.max().item():.3f}, mean={col1.mean().item():.3f}")
            print(f"第2列: min={col2.min().item():.3f}, max={col2.max().item():.3f}, mean={col2.mean().item():.3f}")

            # 对第1列应用sigmoid
            col1_sigmoid = torch.sigmoid(col1)
            print(f"\n第1列 sigmoid后: min={col1_sigmoid.min().item():.3f}, max={col1_sigmoid.max().item():.3f}")

            # 检查是否与最终置信度有关
            if len(out[1]) > 0:
                final_scores = out[1][:, 4]  # 最终检测框的置信度
                print(f"\n最终检测框置信度: min={final_scores.min().item():.3f}, max={final_scores.max().item():.3f}")
                
                # 检查 col1_sigmoid * col2_sigmoid 是否接近最终置信度
                col2_sigmoid = torch.sigmoid(col2)
                product = col1_sigmoid * col2_sigmoid
                print(f"col1_sigmoid * col2_sigmoid 乘积: max={product.max().item():.3f}")
            # # 第一次前向+反向，获取对抗梯度（作为CAM显著性）
            # attack_gradient, _, _, _, _ = fcos_attack_mt(img_path, fcos_model, img_tensor, img_tensor, init_det, args.threshold)
            
            # # 生成CAM补丁掩码（800x800尺寸，用于攻击）
            ...
            #     attack_gradient=attack_gradient
            # )
